...
def main() -> None:
    ...
    parser.add_argument("--ticker", default="SPY", help="ETF ticker to download")
    parser.add_argument("--start", type=pd.Timestamp, help="Start date", nargs="?")
    parser.add_argument("--end", type=pd.Timestamp, help="End date", nargs="?")
    ...
    if args.dry_run:
        ...
        start = args.start or end - pd.Timedelta(days=365)
        ohlcv = fetch_ohlcv(args.ticker, start, end)
        if ohlcv.empty:
            raise SystemExit("No market data returned.")
        dates = ohlcv.index
        contract_data = pd.DataFrame(
            {
                "asset": [args.ticker] * len(dates),
                "date": dates,
                "contract": [args.ticker] * len(dates),
                "price": ohlcv["adj_close"].values,
                "volume": ohlcv["volume"].fillna(0).values,
                "open_interest": ohlcv["volume"].fillna(0).values,
                "expiry": [dates[-1] + pd.Timedelta(days=365)] * len(dates),
            }
        )
        dividend_yield = pd.DataFrame(0.0, index=dates, columns=[args.ticker])
        returns = ohlcv["adj_close"].pct_change().fillna(0.0)
        features = returns.to_frame(name="ret")
        regime_labels = (features["ret"] < 0).astype(int)
        current_weights = pd.Series({args.ticker: 0.0})
        multipliers = pd.Series({args.ticker: 1.0})
        fx_rates = pd.Series({args.ticker: 1.0})
        margin_rates = pd.Series({args.ticker: 0.1})
        returns = returns.to_frame(name=args.ticker)
        cost_estimates = pd.Series(
            [1.0, 2.0, 3.0], index=pd.date_range(dates[-1], periods=3, freq="H")
        )
